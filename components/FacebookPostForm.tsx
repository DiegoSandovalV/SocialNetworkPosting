...
const FacebookPostForm = () => {
  ...
  const handleFacebookSubmit = async (e:any) => {
    ...
    if (!message || (!image && !imageUrl)) {
      alert('Please fill in both the message and either select an image or provide an image URL.');
      return;
    }
 ...
 const handleBothPlatformsPost = async (e:any) => {
  ...
  try {
    ...
    alert('Error posting to one or both platforms. Check the console for details.');
  }
};
  ...
  return (
    <div className="max-w-lg mx-auto mt-10 p-4 bg-white shadow-md rounded-lg">
      <h1 className="text-2xl font-semibold mb-4 text-black">Social Media Post Form</h1>
      <form onSubmit={handleFacebookSubmit} encType="multipart/form-data">
        ...
        <div className="mb-4">
          ...
          <input
            ...
            className="mt-2 px-3 py-2 block w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
          />
        </div>
